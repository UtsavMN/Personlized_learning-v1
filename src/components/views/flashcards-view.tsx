...
export function FlashcardsView() {
            ...
            if (selectedDocId) {
                ...
                if (doc && doc.content) {
                    if (doc && doc.content) {
                        ...
                        toast({ title: "Warning", description: "Document has no text content. Created empty deck.", variant: "destructive" });
                    }
                } else {
                    // Empty Deck (Manual) - Placeholder
    ...
    function StudySession({ deckId, onExit }: { deckId: number, onExit: () => void }) {
        ...
        const handleRate = async (quality: number) => {
            ...
            // Next card logic
            setIsFlipped(false);
            // Small delay to allow flip animation to reset visually if needed, though state change usually handles it
            setTimeout(() => {
